...
- macOS: 生成 `dist/deepcode`，并额外输出 `dist/deepcode-mac` 作为发布文件。
- Windows: 生成 `dist/deepcode.exe`，并额外输出 `dist/deepcode-win.exe` 作为发布文件。

> 说明：该命令会先执行 `npm run build`，再执行当前系统对应的 SEA 可执行文件构建。
